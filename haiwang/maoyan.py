...
def parse_data(html):
    ...
    for item in data:
        comment = {
            ...
            'cityName': item['cityName'] if 'cityName' in item else '',  # 处理cityName不存在的情况，类似于c语言中三目预算符？：
            'content': item['content'].replace('\n', ' ', 20),  # 处理评论内容换行的情况，这里要把replace的max参数放大一些，不然评论如果换行，分析起来会很麻烦
            'score': item['score'],
...
def save_to_txt():
    ...
    while start_time > end_time:
        url = 'http://m.maoyan.com/mmdb/comments/movie/249342.json?_v_=yes&offset=0&startTime=' + start_time.replace(' ', '%20')
        html = None
        '''
        ...
        for item in comments:
            with open('/Users/youpeng/zhizhi/comments.txt', 'a', encoding='utf-8') as f:
...
if __name__ == '__main__':
    # html = get_data(
    #     'http://m.maoyan.com/mmdb/comments/movie/249342.json?_v_=yes&offset=0')
    # comments = parse_data(html)
    # print(comments)
    save_to_txt()